...
除了上述显式的会生成新的合成图层；
还会有隐式的合成图层生成：当一个元素位置是在某个应该位于合成图层的元素之上的时候，这个元素会被隐式的生成新的合成图层

合成图层是运行在合成线程内的，不会受到主线程的影响，可以保证动画流畅
...
1. 图层提升伴随着repaint，需要创建一个新的图层并绘制，然后在原图层上删除提升的元素
2. 渲染数据需要传递给GPU，数据过大可能引起卡顿
3. 会造成隐性的图层提升
4. 会造成额外的内存消耗

解决方案

1. 有动画效果的元素尽可能放在html结构中的最外层，最好不要深层其那套，避免因为产生隐性图层提升
   1. 现实中往往不能满足，可以clone元素，放在body子级，然后专门用来做动画
2. 使用will-change提前提升图层
   1. 不能滥用，避免造成大量内存消耗
3. 减少合成图层的尺寸
   1. 因为这个尺寸会最终影响在GPU中占用的内存
4. 尽量使用css过渡和动画
   1. 尽量不使用js动画

#### CSS动画和JS动画

使用css做动画，声明式的指定动画的开始和结束状态，中间可以完全交给GPU处理，更加的流畅

使用JS做动画，每一帧的处理都是在主线程上的，为了实现流畅的动画效果，必须以至少60帧的频率去计算状态，通知GPU，会跟主线程的其他任务相互阻塞干扰

> 为了实现至少60帧的频率，js动画一般也是需要写在`requestAnimationFrame`内的

### GPU存储一个图形需要多少内存

100px *200px 的红色矩形
需要 100* 200 * 4 = 80000bytes
